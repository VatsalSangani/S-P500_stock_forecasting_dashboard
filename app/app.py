...
FUNDAMENTALS_PATH = "data/fundamentals/fundamentals.parquet"
...
def get_fundamentals(ticker: str):
    """Fetch fundamentals for a given ticker from the parquet file."""
    if not os.path.exists(FUNDAMENTALS_PATH):
        return None
    fundamentals_df = pd.read_parquet(FUNDAMENTALS_PATH)
    record = fundamentals_df[fundamentals_df["Ticker"] == ticker]
    if record.empty:
        return None
...
if df is not None:
    ...
    with tab1:
        ...
        if fundamentals:
            ...
            col1.metric("P/E Ratio", f"{fundamentals['PE_Ratio']:.2f}")
            col2.metric("EPS", f"{fundamentals['EPS']:.2f}")
            col3.metric("Dividend Yield", f"{fundamentals['Dividend_Yield']}")
            col4.metric("Market Cap", f"{fundamentals['Market_Cap'] / 1e9:.2f} B")

            col1.metric("Forward PE", f"{fundamentals['Forward_PE']:.2f}")
            col2.metric("Beta", f"{fundamentals['Beta']:.2f}")
            col3.metric("52W High", f"${fundamentals['52W_High']:.2f}")
            col4.metric("52W Low", f"${fundamentals['52W_Low']:.2f}")
        else:
